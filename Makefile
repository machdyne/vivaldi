...
test_chopin_ml1_v1:
	mkdir -p output
	yosys -q -p "synth_ecp5 -top test -json output/test_chopin_ml1.json" rtl/test_chopin_ml1.v
	nextpnr-ecp5 --45k --package CABGA256 --lpf chopin_ml1_v1.lpf --json output/test_chopin_ml1.json --textcfg output/chopin_ml1_test_out.config
	ecppack -v --compress --freq 2.4 output/chopin_ml1_test_out.config --bit output/chopin_ml1.bit
